...
class Map:
    ...
    MODES = ('tags.any:landscape', 'tags.any:road', 'tags.any:admin')
    ...
    def get_ll_by_click(self, x: float, y: float) -> tuple[float, float]:
        ...
        return (self.ll[0] + delta[1] * real_pixel_size_y, self.ll[1] + delta[0] * real_pixel_size_x)
        
    # def change_ll(self, delta_lon: float, delta_lat: float):
    #     self.ll = (self.ll[0] + delta_lon, self.ll[1] + delta_lat)

    def move_vertically(self, is_up: bool):
        self.ll = (self.ll[0], self.ll[1] + self.scale / 3 * (1 if is_up else -1))

    def move_horizontally(self, is_right: bool):
        self.ll = (self.ll[0] + self.scale / 3 * (1 if is_right else -1), self.ll[1])

    def change_scale(self, scale_up: bool):
        if scale_up:
            self.scale = min(self.scale * 10, 10)

        else:
            self.scale = max(self.scale * 0.1, 0.001)
    ...
    def find(self, geocode: str):
        ...
        self.postcode = address.get('postal_code', '')
    ...
    def find_organisation(self, lon: float, lat: float):
        ...
        self.point = ','.join(map(str, ll)) + ',pm2gnl'
